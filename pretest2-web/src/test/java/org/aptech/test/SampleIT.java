...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "pretest2-ear.ear");

        // Current directory is the root of the "pretest2-web" project. Go up one level
        // and enter the "pretest2-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../pretest2-ejb/target/pretest2-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "pretest2-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/pretest2-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "pretest2-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
